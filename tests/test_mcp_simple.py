...
from pycontextify.mcp_server import reset_manager
...
def mcp_isolated():
    ...
    try:
        ...
        # Cleanup
        try:
            current_manager = mcp_server.get_manager()
            if current_manager:
...
class TestMCPFunctions:
    ...
    def test_status_function(self, mcp_isolated):
        ...
        assert (
            len(status["mcp_server"]["mcp_functions"]) == 7
        )  # Now includes reset_index
...
"""
    ...
    "processing": {
        ...
        "max_workers": 4
            ...
            print(
                f"✅ index_code: {result['files_processed']} files, {result['chunks_added']} chunks"
            )
...
""",
        ...
        for query, description in search_tests:
            ...
            print(
                f"✅ Search '{query}': {len(results)} results, best score: {score:.3f}"
            )
        ...
        with tempfile.NamedTemporaryFile(
            ...
            try:
                ...
                results = mcp_isolated.search_with_context.fn(
                    "authentication system", top_k=3, include_related=True
                )
...
""",
    ...
    "database": {
        ...
        print(
            f"✅ Final status: {final_status['metadata']['total_chunks']} chunks indexed"
        )
