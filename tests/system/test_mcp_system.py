...
class TestMCPServerSystem:
    ...
    def test_complete_user_flow_via_mcp(self, test_environment):
        ...
        try:
            ...
            print(
                f"✅ All {len(expected_tools)} expected tools available (unified API)"
            )
            ...
            doc_result = index_filebase_fn(base_path=str(doc_dir), topic="api_docs")
            ...
            code_result = index_filebase_fn(
                base_path=str(env["code_dir"]), topic="codebase"
            )
            ...
            print(
                f"✅ Codebase indexed: {code_result['files_loaded']} files, {code_result['chunks_created']} chunks"
            )
            ...
            total_expected = (
                doc_result["chunks_created"] + code_result["chunks_created"]
            )
            ...
            for query in test_queries:
                ...
                assert isinstance(
                    results, list
                ), f"Search should return list for '{query}'"
    ...
    def test_mcp_server_help(self):
        ...
        try:
            ...
            try:
                ...
                pytest.fail(
                    f"MCP module tests failed - Parser creation: {e}, Import: {import_e}"
                )
