...
class TestEnhancedIntegration:
    ...
    def test_index_manager_initialization_with_enhancements(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch(
                    "pycontextify.index.embedders.EmbedderFactory.create_embedder"
                ) as mock_embedder_factory,
                patch(
                    "pycontextify.index.manager.VectorStore"
                ...
                with patch(
                    "pycontextify.index.hybrid_search.HybridSearchEngine"
                ) as mock_hybrid_search_class:
                    ...
                    with patch(
                        "pycontextify.index.reranker.CrossEncoderReranker"
                        ...
                        assert (
                            manager.embedder is None
                        )  # Not loaded yet due to lazy loading
                        assert (
                            manager.vector_store is None
                        )  # Not loaded yet due to lazy loading
            ...
            with (
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        if (
                            hybrid_search_stats
                        ):  # Only check if hybrid search is initialized
    ...
    def test_pdf_document_loading_integration(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                with (
                    patch(
                        "pycontextify.index.loaders.DocumentLoader.load"
                    ) as mock_document_load,
                    patch(
                        "pycontextify.index.manager.MetadataStore"
                    ) as mock_metadata_store_class,
                    patch(
                        "pycontextify.index.manager.ChunkerFactory"
                    ) as mock_chunker_factory,
                    patch(
                        "pycontextify.index.manager.RelationshipStore"
                    ) as mock_relationship_store_class,
                ):

                    mock_document_load.return_value = [
                        (
                            fake_pdf_path,
                            "This is extracted PDF text content for testing.",
                        )
                    ]
                    ...
                    mock_metadata_store.get_chunks_by_source_path.return_value = (
                        []
                    )  # No existing chunks for reindexing
                    mock_metadata_store_class.return_value = mock_metadata_store

                    # Mock relationship store
                    mock_relationship_store = Mock()
                    mock_relationship_store.build_relationships_from_chunks.return_value = (
                        None
                    )
                    ...
                    mock_vector_store.add_vectors.return_value = [
                        0
                    ]  # Return list of faiss IDs
            ...
            with (
                ...
                with patch(
                    ...
                    with patch("pycontextify.index.hybrid_search.HybridSearchEngine"):
                        ...
                        mock_chunk.start_char = 0
                        mock_chunk.end_char = 100
                        mock_chunk.created_at.isoformat.return_value = (
                            "2023-01-01T00:00:00"
                        )
                        ...
                        manager.metadata_store.get_all_chunks = Mock(
                            return_value=[mock_chunk]
                        )  # Add this for vector search
                        manager.metadata_store.get_chunk_by_chunk_id = Mock(
                            return_value=mock_chunk
                        )  # Fallback method
                        ...
                        assert (
                            response.success
                        ), f"Search should succeed: {response.error}"
                        assert len(response.results) > 0
                        result = response.results[0]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                with patch(
                    ...
                    mock_hybrid_search_class.side_effect = ImportError(
                    ...
                    with patch(
                        ...
                        mock_reranker_class.side_effect = ImportError(
