...
from pycontextify.index.embedders import BaseEmbedder, EmbedderFactory, SentenceTransformersEmbedder
from pycontextify.index.embedders.base import EmbeddingError, ModelNotFoundError, ProviderNotAvailableError
...
class MockEmbedder(BaseEmbedder):
    ...
    def get_model_info(self):
        return {"provider": self.provider_name, "model": self.model_name, "dimension": self.dimension}
...
class TestEmbedderFactory:
    ...
    def test_validate_provider_config(self):
        ...
        assert EmbedderFactory.validate_provider_config("sentence_transformers", device="cpu", batch_size=32)
        
        with pytest.raises(ValueError, match="Invalid device"):
            EmbedderFactory.validate_provider_config("sentence_transformers", device="invalid")
        
        with pytest.raises(ValueError, match="Invalid batch_size"):
            EmbedderFactory.validate_provider_config("sentence_transformers", batch_size=-1)
...
class TestSentenceTransformersEmbedder:
    ...
    def test_provider_not_available(self):
        ...
        with patch('importlib.import_module', side_effect=ImportError):
            with pytest.raises(ProviderNotAvailableError, match="sentence-transformers not installed"):
                embedder = SentenceTransformersEmbedder("all-MiniLM-L6-v2")
    ...
    def test_validate_model_name(self):
        ...
        assert SentenceTransformersEmbedder.validate_model_name("sentence-transformers/all-mpnet-base-v2")
        assert SentenceTransformersEmbedder.validate_model_name("multi-qa-mpnet-base-dot-v1")
        assert SentenceTransformersEmbedder.validate_model_name("paraphrase-MiniLM-L6-v2")
