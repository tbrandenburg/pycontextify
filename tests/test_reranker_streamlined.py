...
class TestCrossEncoderRerankerStreamlined:
    ...
    def test_initialization_scenarios(self):
        ...
        mock_sentence_transformers.CrossEncoder.side_effect = Exception(
            "Model load failed"
        )
        with patch.dict(
            "sys.modules", {"sentence_transformers": mock_sentence_transformers}
    ...
    def test_successful_reranking_operations(self):
        ...
        reranked_standalone = reranker.rerank(
            "test query", results, combine_scores=False
        )
        ...
        for i in range(len(reranked_standalone) - 1):
            assert (
                reranked_standalone[i].final_score
                >= reranked_standalone[i + 1].final_score
            )
