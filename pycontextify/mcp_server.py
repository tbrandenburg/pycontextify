...
def index_document(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if path_obj.suffix.lower() not in supported_extensions:
            return {"error": f"Unsupported file type. Supported: {', '.join(supported_extensions)}"}
...
@mcp.tool
def index_webpage(url: str, recursive: bool = False, max_depth: int = 1) -> Dict[str, Any]:
    ...
    try:
        ...
        if max_depth > 3:
            max_depth = 3
            logger.warning(f"Limited max_depth to 3 for safety")
...
@mcp.tool
def search_with_context(query: str, top_k: int = 5, include_related: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Context search completed for '{query}': {len(results)} results (include_related={include_related})")
        return results
...
def status() -> Dict[str, Any]:
    ...
    try:
        ...
        result["mcp_server"] = {
            ...
            "interface": "simplified"  # Indicates we're using the simplified 6-function interface
        }
...
def main():
    ...
    try:
        ...
        logger.info("  - index_document(path): Index single document")
        logger.info("  - index_webpage(url, recursive, max_depth): Index web content")
        logger.info("  - search(query, top_k): Basic semantic search")
        logger.info("  - search_with_context(query, top_k, include_related): Enhanced search with relationships")
