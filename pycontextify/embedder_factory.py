...
from .embedder import BaseEmbedder, EmbeddingError, ProviderNotAvailableError
...
class EmbedderFactory:
    ...
    def register_provider(cls, name: str, embedder_class: Type[BaseEmbedder]) -> None:
        ...
        if not issubclass(embedder_class, BaseEmbedder):
            raise ValueError("Provider class must inherit from BaseEmbedder")
    ...
    def _validate_sentence_transformers_config(cls, **kwargs) -> bool:
        ...
        if not isinstance(batch_size, int) or batch_size <= 0:
            raise ValueError(
                f"Invalid batch_size '{batch_size}' for sentence_transformers"
            )
    ...
    def _create_ollama_embedder(
        cls, model_name: str, base_url: str = "http://localhost:11434", **kwargs
    ) -> BaseEmbedder:
    ...
    def _create_openai_embedder(
        cls, model_name: str, api_key: str, **kwargs
    ) -> BaseEmbedder:
