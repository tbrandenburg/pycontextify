...
class RelationshipExtractor(ABC):
    ...
    def extract_relationships(
        self, chunk: ChunkMetadata, entities: Optional[List[Entity]] = None
    ) -> List[Relationship]:
...
class SemanticRelationshipExtractor(RelationshipExtractor):
        ...
        for rel_type, patterns in self._semantic_patterns.items():
            ...
            for pattern in patterns:
                try:
                    ...
                    logger.warning(
                        f"Invalid regex pattern for {rel_type}: {pattern} - {e}"
                    )
            self._compiled_patterns[rel_type] = compiled

        # Entity co-occurrence patterns
        self._cooccurrence_window = (
            100  # Characters within which entities are considered co-occurring
        )
    ...
    def extract_relationships(
        ...
        cooccurrence_relationships = self._extract_cooccurrence_relationships(
            chunk, entities
        )
    ...
    def _extract_pattern_relationships(
        self, chunk: ChunkMetadata, entities: List[Entity]
    ) -> List[Relationship]:
            ...
            for entity2 in entities[i + 1 :]:
                if entity1.source_position and entity2.source_position:
                    ...
                    distance = min(
                        abs(
                            pos1[0] - pos2[1]
                        ),  # Distance from end of entity1 to start of entity2
                        abs(
                            pos2[0] - pos1[1]
                        ),  # Distance from end of entity2 to start of entity1
                    )
                    ...
                    if distance <= self._cooccurrence_window:
                        ...
                        rel_type, source_entity, target_entity = (
                            self._determine_cooccurrence_relationship(
    ...
    def _extract_code_relationships(
        ...
        methods = entities_by_type.get("method", []) + entities_by_type.get(
            "function", []
        )
        ...
        for class_entity in classes:
            for method_entity in methods:
                ...
                if (
                    class_entity.source_position
                    and method_entity.source_position
                    and class_entity.source_position[0]
                    < method_entity.source_position[0]
                ):
        ...
        for import_entity in imports:
            for other_entity in entities:
                if (
                    other_entity.entity_type in ["function", "class", "variable"]
                    and other_entity != import_entity
                ):
                    # Check if the imported module is used
                    import_name = import_entity.name.split(".")[
                        -1
                    ]  # Last part of module name
    ...
    def _extract_document_relationships(
        ...
        for heading in headings:
            for entity in other_entities:
                ...
                if (
                    heading.source_position
                    and entity.source_position
                    and heading.source_position[1] < entity.source_position[0]
                ):
                    ...
                    for other_heading in headings:
                        if (
                            ...
                            and other_heading.source_position
                            and heading.source_position[1]
                            < other_heading.source_position[0]
                            < entity.source_position[0]
                        ):
                            if (
                                not closer_heading
                                or other_heading.source_position[0]
                                < closer_heading.source_position[0]
                            ):
        ...
        for ref_entity in references:
            for other_entity in entities:
                if other_entity != ref_entity and other_entity.entity_type in [
                    "heading",
                    "technical_term",
                ]:
    ...
    def _extract_web_relationships(
        ...
        for url_entity in urls:
            try:
                ...
                domain_entities = [
                    e for e in entities if e.name.lower() == domain.lower()
                ]
    ...
    def _calculate_pattern_confidence(
        ...
        entity_confidence_factor = (
            source_entity.confidence + target_entity.confidence
        ) / 2
    ...
    def _calculate_cooccurrence_confidence(
        self, entity1: Entity, entity2: Entity, distance: int
    ) -> float:
    ...
    def _determine_cooccurrence_relationship(
        ...
        elif source_type == "heading" and target_type in [
            "technical_term",
            "reference",
        ]:
            rel_type = RelationshipType.CONTAINS
        ...
        # Domain relationships
            ...
            "programming_concept",
            "technical_term",
        ] and target_type in ["function", "class"]:
            rel_type = RelationshipType.RELATED_TO
    ...
    def _should_create_generic_relationship(
        ...
        if entity1.entity_type == entity2.entity_type and entity1.entity_type not in [
            "function",
            "class",
            "variable",
            "method",
        ...
        if (
            entity1.entity_type in generic_types
            and entity2.entity_type in generic_types
        ):
            return False
...
class CrossDocumentRelationshipDetector:
    ...
    def find_cross_document_relationships(
        self, chunks: List[ChunkMetadata], entities_by_chunk: Dict[str, List[Entity]]
    ) -> List[Relationship]:
        ...
        for group_entities in entity_groups.values():
            if (
                len(group_entities) > 1
            ):  # Only create relationships if entities appear in multiple chunks
                cross_relationships = self._create_cross_chunk_relationships(
                    group_entities
                )
                relationships.extend(cross_relationships)

        # Find semantic similarity relationships
        semantic_relationships = self._find_semantic_relationships(
            entities_by_chunk, chunks
        )
    ...
    def _create_cross_chunk_relationships(
        self, group_entities: List[Tuple[Entity, str]]
    ) -> List[Relationship]:
    ...
    def _find_semantic_relationships(
        self, entities_by_chunk: Dict[str, List[Entity]], chunks: List[ChunkMetadata]
    ) -> List[Relationship]:
    ...
    def _calculate_semantic_similarity(self, entity1: Entity, entity2: Entity) -> float:
        ...
        if entity1.context and entity2.context:
            ...
            context_similarity = (
                context_intersection / context_union if context_union > 0 else 0.0
            )
