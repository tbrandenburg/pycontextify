...
class SearchResult:
    ...
    position: Optional[Dict[str, Any]] = (
        None  # start_char, end_char, line_start, line_end
    )
    ...
    relevance_explanation: Optional[Dict[str, Any]] = (
        None  # Why this result is relevant
    )
...
class SearchResponse:
            ...
            response.performance = {
                "error_recovery": {
                    ...
                    "partial_results_available": (
                        len(partial_results) > 0 if partial_results else False
                    ),
                }
            }
    ...
    def _format_readable(self) -> str:
        ...
        if not self.results:
            ...
            if self.query_analysis and "enhancements" in self.query_analysis:
                suggestions = self.query_analysis["enhancements"].get(
        ...
        formatted = [
            f"🔍 Found {len(self.results)} result{'s' if len(self.results) != 1 else ''} for '{self.query}'"
        ]
    ...
    def _format_result_readable(self, result: "SearchResult", rank: int) -> str:
        ...
        source_name = self._get_display_source_name(
            result.source_path, result.source_info
        )
        confidence = (
        ...
        source_line = self._format_enhanced_source_line(
            source_icon, source_name, result
        )
        ...
        if (
            ...
            and "match_details" in result.relevance_explanation
        ):
            matched_terms = result.relevance_explanation["match_details"].get(
                "matched_terms", []
            )
            if matched_terms:
        ...
        if result.context and "related_chunks" in result.context:
            ...
            if related_count > 0:
                insights.append(
                    f"📚 {related_count} related chunk{'s' if related_count != 1 else ''} available"
                )
    ...
    def _format_query_insights(self) -> str:
        ...
        if "characteristics" in self.query_analysis:
            ...
            insights.append(
                f"• Query type: {intent.title()} (complexity: {complexity:.2f})"
            )
    ...
    def _get_display_source_name(
        self, source_path: str, source_info: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def _format_enhanced_source_line(
        self, source_icon: str, source_name: str, result: "SearchResult"
    ) -> str:
    ...
    if total_candidates > 0:
        perf_info["search_efficiency"] = {
            ...
            "filtering_ratio": round(
                total_candidates / max(len(components_used), 1), 2
            ),
        }
...
def create_structured_metadata(
    ...
    **additional_metadata: Any,
    ...
    """
    metadata = {"content": {}, "source": {}, "processing": {}}
...
def create_query_analysis(
    ...
    if suggested_alternatives:
        enhancements["suggested_alternatives"] = suggested_alternatives[
            :5
        ]  # Limit suggestions
    if expansion_terms:
...
def create_versioned_response(
    ...
    if requested_version:
        ...
        try:
            ...
            if include_deprecation_warnings and (
                req_major < current_major
                or (req_major == current_major and req_minor < current_minor)
            ):
...
def create_result_ranking_info(
    ...
    ranking_info = {
        ...
        "top_percentile": round(
            (total_results - rank_position + 1) / total_results * 100, 1
        ),
    }
...
class SearchPerformanceLogger:
    ...
    def log_slow_search(
        self, response: "SearchResponse", threshold_ms: int = 1000
    ) -> None:
            ...
            "performance_summary": {
                **self.search_stats,
                ...
                "average_response_time_ms": round(
                    self.search_stats["average_response_time_ms"], 2
                ),
            },
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
