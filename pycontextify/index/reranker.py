...
class CrossEncoderReranker:
    ...
    def rerank(
        ...
        try:
            ...
            # Get rerank scores
            logger.debug(
                f"Reranking {len(query_doc_pairs)} results for query: '{query}'"
            )
            ...
            logger.info(
                f"Reranked {len(search_results)} results to {len(rerank_results)}"
            )
    ...
    def _extract_original_score(self, result: Any) -> float:
        ...
        if hasattr(result, "combined_score"):
            ...
            return float(
                result.get(
                    "score",
                    result.get("combined_score", result.get("vector_score", 0.0)),
                )
            )
        else:
            return 0.0
    ...
    def _convert_to_rerank_results(
        self, search_results: List[Any], query: str
    ) -> List[RerankResult]:
    ...
    def warmup(
        self, sample_query: str = "test query", sample_text: str = "test document"
    ) -> bool:
