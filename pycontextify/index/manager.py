...
class IndexManager:
    ...
    def _auto_save(self) -> None:
        ...
        try:
            ...
            # Save metadata
            self.metadata_store.save_to_file(str(paths["metadata"]), self.config.compress_metadata)
            
            # Save relationships
            self.relationship_store.save_to_file(str(paths["relationships"]), self.config.compress_metadata)
    ...
    def index_codebase(self, path: str) -> Dict[str, Any]:
        ...
        try:
            # Load content
            loader = LoaderFactory.get_loader(SourceType.CODE, max_file_size_mb=self.config.max_file_size_mb)
    ...
    def index_document(self, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            chunks_added = self._process_content(content, file_path, SourceType.DOCUMENT)
    ...
    def index_webpage(self, url: str, recursive: bool = False, max_depth: int = 1) -> Dict[str, Any]:
        """Index web content.
        ...
        Returns:
            Statistics about the indexing operation
        """
        logger.info(f"Starting webpage indexing: {url} (recursive={recursive}, max_depth={max_depth})")
        ...
        try:
            ...
            for page_url, content in pages:
                chunks_added += self._process_content(content, page_url, SourceType.WEBPAGE)
    ...
    def _process_content(self, content: str, source_path: str, source_type: SourceType) -> int:
    ...
    def search_with_context(self, query: str, top_k: int = 5, include_related: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for result in results:
                ...
                for stored_chunk in self.metadata_store.get_all_chunks():
                    if stored_chunk.source_path == result["source_path"] and stored_chunk.chunk_text == result["chunk_text"]:
                        chunk = stored_chunk
                        break
                ...
                if chunk:
                    ...
                    # Find related chunks
                    related_chunks = []
                    for entity in relationships.get("references", [])[:3]:  # Limit to avoid noise
                        related_chunk_ids = self.relationship_store.get_related_chunks(entity)[:2]
                        for chunk_id in related_chunk_ids:
                            ...
                            if related_chunk and related_chunk.chunk_id != chunk.chunk_id:
                                related_chunks.append({
                                    ...
                                    "chunk_text": related_chunk.chunk_text[:200] + "..." if len(related_chunk.chunk_text) > 200 else related_chunk.chunk_text,
                                    "relationship_type": "reference"
                                })
                    
                    result["related_chunks"] = related_chunks[:3]  # Limit related chunks
