...
class IndexManager:
    ...
    def search_with_context(
        ...
        try:
            ...
            if not include_related or not self.config.enable_relationships:
                ...
                if display_format != "structured":
                    base_response.formatted_output = base_response.format_for_display(
                        display_format
                    )
            ...
            for result in base_response.results:
                ...
                if chunk:
                    ...
                    for entity, frequency in top_entities:
                        ...
                        for chunk_id in related_chunk_ids:
                            ...
                            if (
                                ...
                                shared_entities = len(
                                    set(relationships.get("references", []))
                                    & set(
                                        related_chunk.get_relationships().get(
                                            "references", []
                                        )
                                    )
                                )
                                relationship_strength = min(
                                    1.0, (shared_entities + frequency) / 10.0
                                )

                                # Better text preview with smart truncation
                                preview_text = self._create_smart_preview(
                                    related_chunk.chunk_text, entity, 150
                                )

                                # Calculate relevance score based on query similarity
                                relevance_score = self._calculate_context_relevance(
                                    query,
                                    related_chunk.chunk_text,
                    ...
                    related_chunks = sorted(
                        related_chunks, key=lambda x: x["relevance_score"], reverse=True
                    )[:3]
    ...
    def _create_smart_preview(
        self, text: str, highlight_entity: str, max_length: int = 150
    ) -> str:
    ...
    def _calculate_context_relevance(
        self, query: str, chunk_text: str, relationship_strength: float
    ) -> float:
        ...
        try:
            ...
            if self.embedder:
                try:
                    ...
                    chunk_embedding = self.embedder.embed(
                        chunk_text[:512]
            ...
            if query_words and chunk_words:
                keyword_overlap = len(query_words.intersection(chunk_words)) / len(
                    query_words
                )
    ...
    def _extract_knowledge_catalog(self) -> Dict[str, Any]:
        ...
        for chunk in all_chunks:
            ...
            if chunk.source_type.value == "code":
                ...
                for i in range(
                    len(path_parts) - 2, 0, -1
                ):  # Start from parent of file, work backwards
                    ...
                    if (
                        ...
                        looks_like_project = (
                            len(part) > 4  # Longer names
                            or any(char.isupper() for char in part)  # CamelCase
                            or "_" in part
                            or "-" in part  # snake_case or kebab-case
                            or part.startswith("py")  # Python projects
                            or any(
                                x in part.lower()
                                for x in ["project", "app", "service", "api", "tool"]
                            )
                        )
            ...
            # Extract web domains
            if chunk.source_type.value == "webpage" and chunk.source_path.startswith(
                ("http://", "https://")
            ):
                try:
        ...
        content_overview = {
            ...
            "domains": {
                ...
                "codebase_paths": (
                    list(codebases)[:10] if codebases else []
                ),  # Limit to top 10
            },
        }
        ...
        for ref in all_references:
            ...
            if (
                len(ref) > 2
                and ref.lower() not in stop_words
                and not ref.isdigit()
                and len(ref.strip()) > 1
            ):
        ...
        sorted_codebases = sorted(
            codebases, key=len
        )  # Shorter paths first (likely parents)
        ...
        knowledge_catalog = {
            "indexed_sources": {
                ...
                "documents": (
                    list(documents)[:15] if documents else []
                ),  # Increased from 10
            },
            "primary_topics": [
                ref[0] for ref in top_references[:12]
            ],  # Filtered meaningful concepts
            "searchable_concepts": {
                "code_symbols": [
                    symbol[0] for symbol in top_symbols[:15]
                ],  # High-quality symbols
                "technologies": (
                    list(programming_languages) if programming_languages else []
                ),
                "tags": (
                    quality_tags[:12] if quality_tags else []
                ),  # Filtered quality tags
            },
        }
