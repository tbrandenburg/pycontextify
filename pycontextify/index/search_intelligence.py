...
class SearchIntelligenceFramework:
    ...
    def record_search(
        ...
        with self._lock:
            search_record = {
                ...
                "domain": query_analysis.get("domain_analysis", {}).get(
                    "primary_domain", "general"
                ),
                "complexity": query_analysis.get("complexity_analysis", {}).get(
                    "complexity_level", "moderate"
                ),
                "results_count": len(search_response.results),
                "success": search_response.success,
                "response_time": (
                    search_response.performance.get("total_time_ms", 0)
                    if search_response.performance
                    else 0
                ),
                "search_mode": (
                    search_response.performance.get("search_mode", "unknown")
                    if search_response.performance
                    else "unknown"
                ),
    ...
    def get_query_suggestions(
        ...
        similar_queries.sort(
            key=lambda x: (x["score"], x["success_rate"]), reverse=True
        )
    ...
    def get_adaptive_search_recommendations(
    ...
    def analyze_search_patterns(self) -> Dict[str, Any]:
        ...
        if not recent_searches:
            recent_searches = self.search_history[
                -100:
            ]  # Fallback to last 100 searches

        # Pattern analysis
        patterns = {
            "most_common_intents": Counter(
                record["intent"] for record in recent_searches
            ).most_common(5),
            "most_common_domains": Counter(
                record["domain"] for record in recent_searches
            ).most_common(5),
            "complexity_distribution": Counter(
                record["complexity"] for record in recent_searches
            ),
            "success_rate": sum(1 for r in recent_searches if r["success"])
            / len(recent_searches),
            "average_response_time": sum(r["response_time"] for r in recent_searches)
            / len(recent_searches),
        }
        ...
        top_domain = (
            patterns["most_common_domains"][0]
            if patterns["most_common_domains"]
            else None
        )
    ...
    def get_search_quality_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "average_response_time": sum(r["response_time"] for r in recent_searches)
            / len(recent_searches),
            "average_results_per_search": sum(
                r["results_count"] for r in recent_searches
            )
            / len(recent_searches),
        }
        ...
        metrics["performance_by_mode"] = {
            mode: {"average_time": sum(times) / len(times), "count": len(times)}
    ...
    def _update_patterns(
        self, search_record: Dict[str, Any], query_analysis: Dict[str, Any]
    ) -> None:
    ...
    def _update_performance_metrics(self, search_record: Dict[str, Any]) -> None:
        ...
        self.performance_metrics["response_times"].append(
            search_record["response_time"]
        )
        self.performance_metrics["success_rates"].append(
            1.0 if search_record["success"] else 0.0
        )
                ...
                suggestions.append(
                    {
                        ...
                        "domain": (
                            pattern["domains"].most_common(1)[0][0]
                            if pattern["domains"]
                            else "general"
                        ),
                        "intent": (
                            pattern["intents"].most_common(1)[0][0]
                            if pattern["intents"]
                            else "general"
                        ),
    ...
    def _find_successful_patterns(
        self, domain: str, intent: str, complexity: str
    ) -> List[Dict[str, Any]]:
        ...
        if strategy_key in self.successful_strategies:
            ...
            for strategy in strategies:
                ...
                usage_count = sum(
                    1
                    for record in self.search_history[-500:]
    ...
    def _generate_usage_recommendations(
        self, patterns: Dict[str, Any], recent_searches: List[Dict]
    ) -> List[Dict[str, Any]]:
        ...
        for domain, count in top_domains:
            if (
                count > len(recent_searches) * 0.3
            ):  # If domain represents >30% of searches
                recommendations.append(
                    {
    ...
    def _assess_data_quality(self) -> Dict[str, Any]:
        ...
        unique_queries = len(
            set(record["query"] for record in self.search_history[-100:])
        )
        ...
        try:
            ...
            data = {
                ...
                "successful_strategies": {
                    k: list(v) for k, v in self.successful_strategies.items()
                },
    ...
    def _load_intelligence_data(self) -> None:
        ...
        try:
            ...
            self.domain_preferences = defaultdict(
                float, data.get("domain_preferences", {})
            )
            self.successful_strategies = defaultdict(
                list, data.get("successful_strategies", {})
            )
