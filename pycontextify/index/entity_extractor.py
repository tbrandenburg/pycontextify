...
class RegexEntityExtractor(EntityExtractor):
            ...
            for pattern_type, patterns in patterns_dict.items():
                ...
                for pattern in patterns:
                    try:
                        ...
                        logger.warning(f"Invalid regex pattern for {category}.{pattern_type}: {pattern} - {e}")
                    ...
                    for match in pattern.finditer(text):
                        ...
                        if match.groups():
                            if len(match.groups()) == 1:
                                ...
                                entity_name = match.group(-1).strip()  # Last group is usually the entity
                        else:
                            entity_name = match.group(0).strip()
                    
                        if entity_name and len(entity_name) > 1:  # Filter out single characters
    ...
    def _merge_entities(self, entities: List[Entity]) -> Entity:
        ...
        merged_entity = Entity(
            ...
            confidence=min(merged_confidence * 1.1, 1.0),  # Slight boost for multiple occurrences
...
class NEREntityExtractor(EntityExtractor):
            ...
            for ent in doc.ents:
                ...
                entity = Entity(
                    ...
                    attributes={
                        ...
                        'spacy_label_desc': spacy.explain(ent.label_) if 'spacy' in globals() else None
                    }
                )
                entities.append(entity)
    ...
    def get_supported_entity_types(self) -> List[str]:
        ...
        return [
            ...
            'money', 'quantity', 'ordinal', 'cardinal', 'facility'
        ]
    ...
    def extract_entities(self, text: str, source_type: str = "unknown") -> List[Entity]:
        ...
        for extractor in self.extractors:
            try:
                ...
                logger.warning(f"Entity extractor {type(extractor).__name__} failed: {e}")
