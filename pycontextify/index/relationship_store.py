...
class RelationshipStore:
    ...
    def add_relationship(self, entity: str, chunk_id: str, relationship_type: str) -> None:
    ...
    def get_related_chunks(self, entity: str, relationship_type: str = "all") -> List[str]:
    ...
    def get_entity_graph(self, entity: str, max_depth: int = 2) -> Dict[str, Any]:
        ...
        while entities_to_process:
            ...
            for rel_type, chunk_list in self._relationships[current_entity].items():
                graph["nodes"][current_entity]["relationships"][rel_type] = len(chunk_list)
            
            # Find connected entities
            if depth < max_depth:
                for chunk_id in related_chunks:
                    for connected_entity in self._reverse_index[chunk_id]:
                        if connected_entity != current_entity and connected_entity not in processed:
                            # Add edge
                            graph["edges"].append({
                                "from": current_entity,
                                "to": connected_entity,
    ...
    def extract_code_relationships(self, chunk: ChunkMetadata) -> List[Tuple[str, str, str]]:
    ...
    def extract_document_relationships(self, chunk: ChunkMetadata) -> List[Tuple[str, str, str]]:
    ...
    def extract_web_relationships(self, chunk: ChunkMetadata) -> List[Tuple[str, str, str]]:
    ...
    def load_from_file(self, filepath: str) -> None:
        ...
        self._relationships = defaultdict(lambda: defaultdict(list), data.get("relationships", {}))
        self._reverse_index = defaultdict(list, data.get("reverse_index", {}))
        self._entity_chunks = defaultdict(set, {
            k: set(v) for k, v in data.get("entity_chunks", {}).items()
        })