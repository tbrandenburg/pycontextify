...
class HybridSearchEngine:
    ...
    def search(
        ...
        results = []
        for faiss_id, vector_score in vector_scores[:top_k * 2]:  # Get more candidates for reranking
            try:
                ...
                result = SearchResult(
                    ...
                    source_type=chunk_metadata.source_type.value if hasattr(chunk_metadata.source_type, 'value') else str(chunk_metadata.source_type),
                    metadata=self._extract_metadata(chunk_metadata)
                )
                results.append(result)
                
            except Exception as e:
                logger.warning(f"Error processing search result for faiss_id {faiss_id}: {e}")
                continue
    ...
    def _tfidf_search(self, query: str) -> Dict[int, float]:
        ...
        try:
            ...
            return {i: float(score) for i, score in enumerate(similarities) if score > 0}
    ...
    def _bm25_search(self, query: str) -> Dict[int, float]:
        ...
        try:
            ...
            # Return scores as dictionary
            return {i: float(score) for i, score in enumerate(normalized_scores) if score > 0}
    ...
    def _extract_metadata(self, chunk_metadata) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "source_path": chunk_metadata.source_path,
            "chunk_id": chunk_metadata.chunk_id,
            "created_at": chunk_metadata.created_at.isoformat() if hasattr(chunk_metadata.created_at, 'isoformat') else str(chunk_metadata.created_at),
        }
        ...
        if hasattr(chunk_metadata, 'embedding_provider') and chunk_metadata.embedding_provider:
            metadata["embedding_provider"] = chunk_metadata.embedding_provider
            
        if hasattr(chunk_metadata, 'embedding_model') and chunk_metadata.embedding_model:
