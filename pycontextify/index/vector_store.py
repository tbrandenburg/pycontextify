...
class VectorStore:
    ...
    def search(self, query_vector: np.ndarray, top_k: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Search
        distances, indices = self._index.search(query_vector, min(top_k, self._total_vectors))
    ...
    def load_from_file(self, filepath: str) -> None:
        ...
        if not filepath.exists():
            logger.info(f"Index file {filepath} does not exist, starting with empty index")
            return
        ...
        try:
            ...
            if self._index.d != self.dimension:
                raise ValueError(f"Loaded index dimension {self._index.d} doesn't match expected {self.dimension}")
            
            logger.info(f"Loaded FAISS index from {filepath} with {self._total_vectors} vectors")
        except Exception as e:
