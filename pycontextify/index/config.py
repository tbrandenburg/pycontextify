...
class Config:
    ...
    def __init__(self, env_file: Optional[str] = None) -> None:
        ...
        self.compress_metadata = self._get_bool_config("PYCONTEXTIFY_COMPRESS_METADATA", True)
        self.backup_indices = self._get_bool_config("PYCONTEXTIFY_BACKUP_INDICES", False)
        self.max_backups = self._get_int_config("PYCONTEXTIFY_MAX_BACKUPS", 3)
        
        # Embedding provider configuration
        self.embedding_provider = os.getenv("PYCONTEXTIFY_EMBEDDING_PROVIDER", "sentence_transformers")
        self.embedding_model = os.getenv("PYCONTEXTIFY_EMBEDDING_MODEL", "all-mpnet-base-v2")
        
        # Provider-specific settings
        self.ollama_base_url = os.getenv("PYCONTEXTIFY_OLLAMA_BASE_URL", "http://localhost:11434")
        ...
        self.enable_relationships = self._get_bool_config("PYCONTEXTIFY_ENABLE_RELATIONSHIPS", True)
        self.max_relationships_per_chunk = self._get_int_config("PYCONTEXTIFY_MAX_RELATIONSHIPS_PER_CHUNK", 10)
        
        # Performance settings (optional)
        self.max_file_size_mb = self._get_int_config("PYCONTEXTIFY_MAX_FILE_SIZE_MB", 10)
        self.batch_size = self._get_int_config("PYCONTEXTIFY_BATCH_SIZE", 32)
        self.crawl_delay_seconds = self._get_int_config("PYCONTEXTIFY_CRAWL_DELAY_SECONDS", 1)
