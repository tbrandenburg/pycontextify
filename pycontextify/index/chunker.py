...
class BaseChunker(ABC):
    ...
    @abstractmethod
    def chunk_text(self, text: str, source_path: str, embedding_provider: str, 
                   embedding_model: str) -> List[ChunkMetadata]:
    ...
    def _create_chunk_metadata(self, chunk_text: str, source_path: str, 
                              source_type: SourceType, start_char: int, end_char: int,
...
class CodeChunker(SimpleChunker):
    ...
    def chunk_text(self, text: str, source_path: str, embedding_provider: str,
        ...
        for code_text, start_char, end_char in code_chunks:
            ...
            if estimated_tokens <= self.chunk_size:
                ...
                sub_chunks = self._split_by_tokens(code_text, self.chunk_size, self.chunk_overlap)
    ...
    def _split_by_code_boundaries(self, text: str) -> List[tuple]:
        ...
        for i, line in enumerate(lines):
            ...
            is_boundary = (
                line.strip().startswith(('def ', 'class ', 'function ', 'const ', 'var ', 'let ')) or
                (line.strip().startswith(('public ', 'private ', 'protected ')) and 
                 any(keyword in line for keyword in ['function', 'class', 'def']))
            )
    ...
    def _extract_relationships(self, chunk: ChunkMetadata) -> None:
        ...
        var_pattern = r'(?:var|let|const|[a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*'
        var_names = re.findall(r'^(?:var|let|const)\s+([a-zA-Z_][a-zA-Z0-9_]*)', text, re.MULTILINE)
    ...
    def chunk_text(self, text: str, source_path: str, embedding_provider: str,
        ...
        for chunk_text, start_char, end_char, section in doc_chunks:
            ...
            if estimated_tokens <= self.chunk_size:
                ...
                sub_chunks = self._split_by_tokens(chunk_text, self.chunk_size, self.chunk_overlap)
                for sub_text, sub_start, sub_end in sub_chunks:
                    adjusted_start = start_char + sub_start
                    adjusted_end = start_char + sub_end
                    final_chunks.append((sub_text, adjusted_start, adjusted_end, section))
    ...
    def _split_by_document_structure(self, text: str) -> List[tuple]:
        ...
        for line in lines:
            ...
            if header_match:
                ...
                if current_chunk_lines and len(' '.join(current_chunk_lines).strip()) > 50:
                    chunk_text = '\n'.join(current_chunk_lines)
                    end_char = char_position - 1
                    chunks.append((chunk_text, current_start_char, end_char, current_section))
        ...
        if len(chunks) <= 1:
            simple_chunks = self._split_by_tokens(text, self.chunk_size, self.chunk_overlap)
            return [(chunk_text, start_char, end_char, None) 
...
class WebPageChunker(DocumentChunker):
    ...
    def chunk_text(self, text: str, source_path: str, embedding_provider: str,
        ...
        for chunk_text, start_char, end_char, section, metadata in html_chunks:
            ...
            estimated_tokens = len(chunk_text.split()) * 1.3
            if estimated_tokens <= self.chunk_size:
                final_chunks.append((chunk_text, start_char, end_char, section, metadata))
            else:
                # Split large chunks further
                sub_chunks = self._split_by_tokens(chunk_text, self.chunk_size, self.chunk_overlap)
                for sub_text, sub_start, sub_end in sub_chunks:
                    adjusted_start = start_char + sub_start
                    adjusted_end = start_char + sub_end
                    final_chunks.append((sub_text, adjusted_start, adjusted_end, section, metadata))
