...
from .advanced_relationships import Relationship, RelationshipType, SemanticRelationshipExtractor
...
class GraphEdge:
    ...
    def add_context_reference(self, chunk_id: str, context: Optional[str] = None) -> None:
    ...
    def get_strength(self) -> float:
        ...
        frequency_score = min(1.0, len(self.chunk_ids) / 5.0)  # Normalize to max 5 occurrences
...
class KnowledgeGraph:
    ...
    def _build_from_relationship_store(self) -> None:
        ...
        try:
            ...
            logger.info(f"Processing {stats['total_entities']} entities and {stats['total_relationships']} relationships")
            
            # Add nodes for all entities
            # Note: This is a simplified approach - in a real implementation,
            # we'd need access to the actual entity data structure
            all_entities = self.relationship_store._entities.keys() if hasattr(self.relationship_store, '_entities') else []
    ...
    def add_entity(self, entity: Entity, chunk_id: str) -> str:
        ...
        if entity_id in self.nodes:
            ...
            node.confidence = ((node.confidence * old_weight) + entity.confidence) / total_weight
        else:
            # Create new node
            node = GraphNode(
                entity_id=entity_id,
    ...
    def add_relationship(self, relationship: Relationship, chunk_id: str) -> None:
        ...
        if edge_id in self.edges:
            ...
            total_weight = old_weight + 1
            edge.confidence = ((edge.confidence * old_weight) + relationship.confidence) / total_weight
        else:
            # Create new edge
            edge = GraphEdge(
                source_id=source_id,
    ...
    def get_node_neighbors(self, entity_id: str, relationship_types: Optional[List[RelationshipType]] = None) -> List[Tuple[str, GraphEdge]]:
        ...
        for neighbor_id in self.adjacency_list.get(entity_id, set()):
            ...
            if edge and (not relationship_types or edge.relationship_type in relationship_types):
                neighbors.append((neighbor_id, edge))
        
        # Sort by edge strength
        return sorted(neighbors, key=lambda x: x[1].get_strength(), reverse=True)
    
    def find_shortest_path(self, source_entity: str, target_entity: str, max_depth: int = 6) -> Optional[GraphPath]:
        ...
        while queue:
            ...
            if current_id == target_id:
                ...
                total_confidence = sum(edge.confidence for edge in edges) / len(edges) if edges else 1.0
                return GraphPath(
                    nodes=path,
                    edges=edges,
    ...
    def get_entity_subgraph(self, entity_name: str, depth: int = 2, min_confidence: float = 0.3) -> Dict[str, Any]:
    ...
    def discover_clusters(self, min_cluster_size: int = 3, coherence_threshold: float = 0.5) -> List[KnowledgeCluster]:
        ...
        for entity_type, entities in entity_type_groups.items():
            if len(entities) >= min_cluster_size:
                ...
                central_entity_id = max(entities, key=lambda e: centrality_scores.get(e, 0))
                ...
                coherence_score = total_connections / possible_connections if possible_connections > 0 else 0.0
    ...
    def get_recommendations(self, entity_name: str, top_k: int = 5) -> List[Tuple[str, float, str]]:
        ...
        for neighbor_id, _ in self.get_node_neighbors(entity_id):
            for second_neighbor_id, second_edge in self.get_node_neighbors(neighbor_id):
                if second_neighbor_id != entity_id and second_neighbor_id not in [n[0] for n, _ in self.get_node_neighbors(entity_id)]:
                    second_neighbor_name = self.nodes[second_neighbor_id].entity_name
                    score = second_edge.get_strength() * 0.7  # Reduce score for indirect connection
                    reason = f"Connected via {self.nodes[neighbor_id].entity_name}"
                    
                    if second_neighbor_name not in recommendations or recommendations[second_neighbor_name][0] < score:
                        recommendations[second_neighbor_name] = (score, reason)
        
        # Cluster-based recommendations
        node = self.nodes[entity_id]
        if node.cluster_id:
            cluster = next((c for c in self._clusters if c.cluster_id == node.cluster_id), None)
            if cluster:
                for cluster_entity_id in cluster.entities:
                    if cluster_entity_id != entity_id:
                        ...
                        if cluster_entity_name not in recommendations or recommendations[cluster_entity_name][0] < score:
                            recommendations[cluster_entity_name] = (score, reason)
        
        # Sort by score and return top_k
        sorted_recommendations = sorted(
            [(name, score, reason) for name, (score, reason) in recommendations.items()],
            key=lambda x: x[1],
            reverse=True
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "statistics": {
                "avg_node_confidence": sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0.0,
                "avg_relationship_strength": sum(relationship_strengths) / len(relationship_strengths) if relationship_strengths else 0.0,
    ...
    def import_from_dict(self, graph_data: Dict[str, Any]) -> None:
        ...
        logger.info(f"Imported knowledge graph with {len(self.nodes)} nodes and {len(self.edges)} edges")
...
def build_knowledge_graph_from_chunks(
    ...
    if entity_extraction_enabled:
        ...
        for chunk in chunks:
            try:
                ...
                extraction_result = extract_entities_from_text(chunk.chunk_text, source_type)
                ...
                relationships = relationship_extractor.extract_relationships(chunk, entities)
    ...
    stats = kg.get_stats()
    logger.info(f"Knowledge graph built: {stats['total_nodes']} nodes, {stats['total_edges']} edges, {stats['clusters']} clusters")
