...
class PDFLoader:
    ...
    def load_pdf(self, file_path: str) -> str:
        ...
        if self.preferred_engine in self.available_engines:
            try:
                ...
                if text.strip():
                    logger.info(
                        f"Successfully extracted text using {self.preferred_engine}"
                    )
                    return text
                else:
                    logger.warning(
                        f"{self.preferred_engine} extracted empty text, trying fallback"
                    )
        ...
        for engine in self.available_engines:
            ...
            try:
                ...
                if text.strip():
                    logger.info(
                        f"Successfully extracted text using fallback engine: {engine}"
                    )
                    return text
            except Exception as e:
    ...
    def extract_page_context(self, text: str) -> dict:
        ...
        context = {"page_number": None, "section_title": None, "has_page_marker": False}
